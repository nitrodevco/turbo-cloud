...
public sealed partial class RoomGrain
{
    public async Task<bool> AddItemAsync(IRoomItem item, CancellationToken ct)
    {
        ...
        {
            _logger.LogError(ex, "Failed to add an item to room {RoomId}", _state.RoomId);
    ...
    )
    {
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Failed to remove item {ItemId} from room {RoomId} for player {PlayerId}",
                itemId,
                _state.RoomId,
                ctx.PlayerId
            );
    ...
    )
    {
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Failed to use item {ItemId} in room {RoomId} for player {PlayerId}",
                itemId,
                _state.RoomId,
                ctx.PlayerId
            );
